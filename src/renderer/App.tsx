...
function App() {
  ...
  useEffect(() => {
    void api.getPlatform().then(setPlatform).catch(() => undefined)
    void api.isMaximized().then(setMaximized).catch(() => undefined)
  ...
  const activeRecords = useMemo(
    () => (data?.records ?? []).filter((record) => record.typeId === activeType?.id),
    [activeType?.id, data?.records]
  )
  const monthSummary = useMemo(
    () => summarizeMonth(activeRecords, selectedYear, selectedMonth),
  ...
  const requestTypeDelete = (billType: BillType) => {
    ...
    setConfirmDialog({
      ...
      message: recordCount
        ? `此操作将同时永久删除该类型下的 ${recordCount} 条账单，无法撤销。`
        : '该类型目前没有账单，删除后无法撤销。',
  ...
  if (!data || !activeType) {
    return <div className="boot-screen">{loadError ? `加载失败：${loadError}` : <><span className="boot-mark"><ReceiptText /></span>TaroBill</>}</div>
  }
      ...
      <main className="app-shell">
        <aside className="sidebar">
          {!isWindows && (
            <div className="traffic-lights">
              <button className="traffic red" title="关闭" onClick={() => void api.closeWindow()}><X size={8} /></button>
              <button className="traffic yellow" title="最小化" onClick={() => void api.minimizeWindow()}><Minus size={8} /></button>
              <button className="traffic green" title="最大化" onClick={() => void api.toggleMaximize()}><MoveDiagonal2 size={7} /></button>
            </div>
          )}
          ...
          <div className="sidebar-heading">
            <Tag size={18} />
            <span>账单类型</span>
            <button title="新增类型" onClick={() => setTypeEditor(null)}><Plus size={16} /></button>
          </div>
              ...
              return (
                <div className={active ? 'type-row active' : 'type-row'} key={billType.id}>
                  ...
                  <div className="type-actions">
                    <button title="重命名" onClick={() => setTypeEditor(billType)}><Pencil size={14} /></button>
                    <button title="删除" onClick={() => requestTypeDelete(billType)}><Trash2 size={14} /></button>
                  </div>
                </div>
              )
            })}
        ...
        <section className="workspace">
          <header className="topbar">
            <div className="page-title"><h1>{activeType.name}</h1></div>
            <div className="topbar-actions no-drag">
              <div className="period-navigator">
                <button className="icon-button" title="上个月" onClick={() => shiftPeriod(-1)}><ChevronLeft size={17} /></button>
                <strong>{selectedYear}年{selectedMonth + 1}月</strong>
                <button className="icon-button" title="下个月" onClick={() => shiftPeriod(1)}><ChevronRight size={17} /></button>
                <button className="today-button" onClick={resetPeriod}>本月</button>
              </div>
