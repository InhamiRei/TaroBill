        ...
        <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="本月每日支出折线图">
          ...
          {points.map((point, index) => (
            <g key={index} className="line-point">
              <title>
                {index + 1}日：{formatCompactCny(point.value)}
              </title>
              {point.value > 0 && (
                <circle className={index === peakIndex ? 'peak' : ''} cx={point.x} cy={point.y} r={index === peakIndex ? 4 : 2.8} />
              )}
            </g>
          ))}
          {getTickDays(values).map((day) => (
            <text key={day} x={chartLeft + (day - 1) * horizontalStep} y="168" textAnchor="middle">
              {day}日
            </text>
          ))}
        </svg>
    ...
    <section className="panel chart-panel weekly-chart-panel">
      ...
      <div className="weekly-chart" role="img" aria-label="以参考日为中心的五周支出柱状图">
        {entries.map((entry) => {
          ...
          return (
            <div
              className={entry.current ? 'weekly-column current' : 'weekly-column'}
              key={entry.label}
              title={`${entry.label}：${formatCompactCny(entry.value)}`}
            >
              <small>{entry.value ? formatCompactCny(entry.value) : '—'}</small>
              <div className="weekly-bar-track">{entry.value > 0 && <i style={{ height: `${Math.max(heightRatio * 100, 12)}%` }} />}</div>
