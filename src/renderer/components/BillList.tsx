  ...
  return (
    <section className="records-panel">
      <header className="records-header">
        <h2>账单明细</h2>
        <button className="icon-button records-add" title="新增账单" onClick={onAdd}><Plus size={19} /></button>
      </header>
      <div className="records-scroll">
        {records.map((record) => (
          <article className="record-card" key={record.id}>
            <div className="record-icon"><RecordIcon name={record.icon} typeId={record.typeId} /></div>
            ...
            <div className="record-side">
              ...
              <div className="record-actions">
                <button title="编辑账单" onClick={() => onEdit(record)}><Pencil size={15} /></button>
                <button title="删除账单" onClick={() => onDelete(record)}><Trash2 size={15} /></button>
              </div>
            </div>
          </article>
        ))}
